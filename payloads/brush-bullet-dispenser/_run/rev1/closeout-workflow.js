export const meta = {
  name: 'dispenser-closeout',
  description: 'Close out rev-1 dispenser CAD: 0.2mm pilot web fix, BOM strings, clearance re-check, fresh verify, package',
  phases: [
...
const CTX = `You are part of the brush-bullet dispenser close-out run (agricultural granule-metering payload for a UAS; pure CAD work). Repo: /Users/hex/projects/payload-systems, branch brush-bullet-dispenser (already checked out — do NOT switch branches, do NOT push). Package root: ${ROOT}. Read ${R1}/CONTEXT.md for standing directives and ${R1}/VERIFY.md for the independent verification this run must close. The r12 exports in ${ROOT}/cad/exports/ are the shipped baseline; your work produces export tag r13. Every claim of "fixed" or "measured" must quote the printed output of an actual geometry check (OCP/trimesh), never a narrated summary. `
...
for (let round = 1; round <= 2; round++) {
  phase('Fix')
  await agent(CTX + `CAD builder, close-out round ${round}. ${round > 1 ? `A fresh verifier FAILED round 1 — read ${R1}/CLOSEOUT-VERIFY-r1.md and fix every blocking item. ` : ''}Work items, all from ${R1}/VERIFY.md (re-read the full sections, not just this summary):
...
2. B3.4 — state the gearbox pilot-bore diameter (verifier measured Ø16.20) in ${ROOT}/cad/BOM.md with the gearbox datasheet citation; reconcile the ~4 BOM strings VERIFY.md calls out as absent/stale (search VERIFY.md for 'BOM' findings, including the ECO-4 window deviation note — make BOM text match modelled geometry or record the deviation explicitly).
...
4. B6.5/B6.6 NOT VERIFIED — re-run ground and prop clearance against the real airframe/landing-gear STEPs (the rev-0 run used them; find the path in ${ROOT}/_run/CONTEXT.md or rev-0 build notes, likely under /Users/hex/projects/project-quiver). Print measured minima for: ground clearance at rest, payload-to-gear, prop vertical, prop in-plan. If the STEPs genuinely cannot be found, say so explicitly — do not substitute the r6 notes' numbers.
Re-export STEP+STL as *_r13.* for every changed part plus the assembly, run the export-integrity check (watertight, STEP-vs-STL volume) on ALL r13 exports, and confirm no regression by re-running the fragment-jam geometry check and the roof-slot march on r13. Renders: v1r7_iso/section/detail.png to ${ROOT}/cad/renders/, cream background. Write ${R1}/BUILD-NOTES-closeout-r${round}.md with every check's verbatim output.`,
    { label: `fix-r${round}`, phase: 'Fix' })

  phase('Verify')
  verdict = await agent(CTX + `Fresh-eyes verifier, close-out round ${round}. You have NOT seen the build rounds; do not read build notes except to test their claims after you have your own numbers. Do not read or run cad/dispenser.py or the project's own verify scripts — measure the ${ROOT}/cad/exports/*_r13.* files directly with your own OCP/trimesh code. Verify each close-out item from ${R1}/VERIFY.md:
...
- B3.4 + BOM strings: read ${ROOT}/cad/BOM.md and check the pilot-bore statement against your own measurement of the r13 geometry, and spot-check the other corrected strings against measured geometry.
- B7.1: check the published README/docs numbers against your own measurement of the r13 fill/support geometry.
- B6.5/B6.6: independently re-measure ground/prop clearances against the same airframe STEPs the builder cites (state the file paths you used).
...
Write ${R1}/CLOSEOUT-VERIFY-r${round}.md with all measured numbers. Blocking = any close-out item not closed by YOUR measurement, any regression, or any doc claim contradicting your numbers.`,

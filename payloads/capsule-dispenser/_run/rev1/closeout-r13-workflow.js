export const meta = {
  name: 'metering-disc-closeout-r13',
...
for (let round = 1; round <= 2; round++) {
  phase('Fix')
  await agent(CTX + `CAD builder, fix round ${round}. ${round > 1 ? `A fresh verifier FAILED round 1 — read ${R1}/CLOSEOUT-R13-VERIFY-r1.md and fix every blocking item. ` : ''}Work items, all from ${R1}/VERIFY.md (re-read the full sections, not just this summary):
...
Re-export STEP+STL as *_r13.* for every changed part plus the assembly, run the export-integrity check (watertight, STEP-vs-STL volume) on ALL r13 exports, and confirm no regression by re-running the fragment-jam geometry check and the roof-slot march on r13. Renders: v1r7_iso/section/detail.png to ${ROOT}/cad/renders/, cream background. Write ${R1}/BUILD-NOTES-closeout-r13-r${round}.md with every check's verbatim output.`,
...
Write ${R1}/CLOSEOUT-R13-VERIFY-r${round}.md with all measured numbers. Blocking = any close-out item not closed by YOUR measurement (B6.5/B6.6 confirmed-absent STEPs count as open-but-nonblocking-for-this-run), any regression, or any doc claim contradicting your numbers.`,
